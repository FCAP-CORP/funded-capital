...
check("plain part = body, a blank line, then the signature text (no '-- ', as Gmail sends it)", textPart === `Hi Tony,\n\nThanks — talk soon.\n\n${sigText}`, JSON.stringify(textPart.slice(-40)));
check("html part carries the signature HTML in Gmail's own wrapper", htmlPart.includes('<div dir="ltr" class="gmail_signature" data-smartmail="gmail_signature">') && htmlPart.includes("<b>Luis Fajardo</b>"), "");
check("…with no '-- ' line (Luis's Gmail does not add one)", !htmlPart.includes("gmail_signature_prefix") && !htmlPart.includes("-- "), "");
check("…and the signature HTML is passed through untouched", htmlPart.includes(sig), "byte for byte");
